...
class SolutionCPipeline:
    ...
    # Label pattern: text starting with a short label followed by a colon,
    # e.g. "尺寸/size：...", "材质详解：...", "温馨提示：..."
    _label_re = re.compile(r'^[^：:]{1,15}[：:]')

    @staticmethod
    def _has_cjk(text: str) -> bool:
        return any('\u4e00' <= ch <= '\u9fff' for ch in text)

    def merge_paragraph_lines(self, regions: List[TextRegion],
                              max_line_gap_ratio: float = 1.5) -> List[TextRegion]:
        """Merge consecutive text lines that form a single paragraph.

        Regions on adjacent lines with overlapping x-ranges and small
        vertical gaps are merged into one region so the translator sees
        full paragraph context.  Only translatable regions are merged;
        preserved regions (specs, brands) are left untouched.

        Guards prevent merging:
        - Bilingual title (one pure-English + one pure-CJK line)
        - A new labelled section starting after an unlabelled line

        On merge, ``bbox_poly``/``style_info``/``angle`` are cleared so
        the eraser covers the full merged bbox (not just the first
        region's polygon) and the renderer re-extracts style from the
        merged area instead of reusing the first region's style.
        """
        translatable = [r for r in regions if r.is_translatable]
        preserved = [r for r in regions if not r.is_translatable]


        # Sort by vertical position, then horizontal

        translatable.sort(key=lambda r: (r.bbox[1], r.bbox[0]))
        ...
        for region in translatable:
            ...
            if merged:
                ...
                # X overlap ratio (at least 50% of the narrower line)

                ox1, ox2 = max(px1, cx1), min(px2, cx2)
                ...
                x_ratio = overlap / min_w if min_w > 0 else 0

                # Vertical gap relative to current line height

                curr_h = cy2 - cy1

                v_gap = cy1 - py2

                # Don't merge a much narrower previous line that starts at a

                # very different x position (side-by-side parameter, not a

                # paragraph continuation).

                narrow_misaligned = (prev_w < curr_w * 0.4
                                     and abs(px1 - cx1) > curr_w * 0.2)

                # --- Conservative guards ---
                # 1. Don't merge bilingual title (one English, one CJK)
                if self._has_cjk(prev.text) != self._has_cjk(region.text):
                    merged.append(region)
                    continue
                # 2. Don't merge if current line starts with a label
                #    pattern ("label：") — it begins a new section.
                if self._label_re.match(region.text):
                    merged.append(region)
                    continue

                if (x_ratio > 0.5 and not narrow_misaligned
                        and 0 < v_gap < curr_h * max_line_gap_ratio):
                    prev.bbox = [min(px1, cx1), min(py1, cy1),
                                 max(px2, cx2), max(py2, cy2)]
                    prev.text = prev.text + region.text
                    prev.confidence = min(prev.confidence, region.confidence)
                    # Store single-line height before clearing style so the
                    # renderer sizes the font to one line, not the total bbox.
                    if not prev.style_info.get("line_height"):
                        prev.style_info["line_height"] = int(py2 - py1)
                    line_h = prev.style_info["line_height"]
                    # Clear per-region metadata so the eraser covers the
                    # full merged bbox and the renderer re-extracts style.
                    prev.bbox_poly = None
                    prev.style_info = {"line_height": line_h}
                    prev.angle = 0.0
                    continue
            merged.append(region)


        # Re-sort all regions by position for consistent ordering

        result = merged + preserved
    ...
    def process_single_image(
        ...
        regions = self.merge_paragraph_lines(regions)
